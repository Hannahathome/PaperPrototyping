...
// Sidebar main tabs. Assem is last because it is the only conditional one -- it appears
// only in assembly mode, so every other index stays put whether it is there or not.
// mainTabCount() and MAIN_TAB_LABELS are the single source of truth for how many there are.
final int MAIN_TAB_SHAPE   = 0;
final int MAIN_TAB_TEXTURE = 1;
