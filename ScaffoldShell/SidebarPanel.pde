...
class SidebarPanel {
  ...
  void setupMainTabs() {
    // Tab count derives from the label array rather than a literal, so adding a tab is one
    // edit. It used to be `assemblyMode ? 4 : 3` against a 4-entry array, which meant the
    // count and the labels had to be kept in agreement by hand.
    int nTabs = mainTabCount();
    float tabWidth = (w - (nTabs + 1) * SIDEBAR_PADDING) / nTabs;
    float tabHeight = 36;
    float sx = x + SIDEBAR_PADDING;
    float sy = y + SIDEBAR_PADDING;

    String[] tabLabels = MAIN_TAB_LABELS;
    color[] tabColors = {
      color(100, 180, 220),  // Blue for Shape
      color(190, 80, 140),   // Pink/Magenta for Texture
      color(130, 190, 100),  // Green for Print
      color(225, 145, 60),   // Orange for Frame
  ...
  void refreshTabsForAssemblyMode() {
    if (!assemblyMode && activeMainTab == MAIN_TAB_ASSEM) activeMainTab = MAIN_TAB_SHAPE;
    if (assemblyMode) activeMainTab = MAIN_TAB_ASSEM;  // jump straight to Assem tab
    mainTabs.clear();
    setupMainTabs();
  ...
  // Main Draw Function 
    ...
    if (activeMainTab == MAIN_TAB_SHAPE) {
      drawShapeControlContent();
    } else if (activeMainTab == MAIN_TAB_TEXTURE) {
      drawTextureContent();
    } else if (activeMainTab == MAIN_TAB_PRINT) {
      drawViewControlContent();
    } else if (activeMainTab == MAIN_TAB_FRAME) {
      drawFrameContent();
    } else if (activeMainTab == MAIN_TAB_ASSEM) {
      drawAssemblyContent();
    }
  ...
  boolean handleCutoutControlClick() {
    if (activeMainTab != MAIN_TAB_PRINT) return false;
  ...
  boolean mousePressed() {
    ...
    // Frame tab: every control on it is hand-drawn, so it does its own hit-testing against
    // the same row list it drew from. See FrameSidebar.pde.
    if (activeMainTab == MAIN_TAB_FRAME) {
      return handleFrameTabClick();
    }
    ...
    if (activeMainTab == MAIN_TAB_PRINT) {
    ...
    if (activeMainTab == MAIN_TAB_ASSEM) {
    ...
    if (activeMainTab != MAIN_TAB_TEXTURE) {
      return false;
    }
  ...
  boolean handleMouseWheel(float count) {
    if (activeMainTab == MAIN_TAB_FRAME) return handleFrameTabWheel(count);
    if (activeMainTab != MAIN_TAB_TEXTURE || activeTextureTab != TEX_TAB_PER_PANEL) return false;
